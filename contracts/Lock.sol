...
contract TokenTimelockProxy {
    ...
    /**
     * @dev Emitted when the a `locker` locks an `amount` of tokens for 
     * a `beneficiary` until the `releaseTime` through a newly generated 
     * Timelock contract at the address `timelockContractAddress` 
     */
    event TokensLocked(address indexed locker, address indexed beneficiary, uint256 amount, uint256 releaseTime, address indexed timelockContractAddress);
    ...
    function lockTokens(
        ...
        // Checks if the msg sender has allowed this contract to transfer 
        // the amount amountToLock_
        require(
            IERC20Upgradeable(_tokenToLock).allowance(
                msg.sender,
                address(this)
            ) >= amountToLock_,
            "TokenTimelockProxy: Insufficient Allowance"
        );

        // Creates a new TimeLock Contract by making a clone
        address clone = ClonesUpgradeable.clone(_implementation);
        ...
        // Transfers the amount amountToLock_ to the TimeLock Contract
        ...
        // Stores contract clone reference
        _clonedContracts.push(clone);
        _clonedContractsPerUser[msg.sender].push(_clonedContracts.length - 1);

        emit TokensLocked(msg.sender,beneficiary_,amountToLock_,releaseTime_,clone);
